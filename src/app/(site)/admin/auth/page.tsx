...
import { LayoutDashboard, Eye, EyeOff, Loader2, AlertTriangle, User } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { useUser, useAuth, initiateEmailSignIn, initiateEmailSignUp, useFirestore } from '@/firebase';
...
export default function AuthPage() {
  ...
  const handleAdminSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Store additional user info in Firestore 'admin_users' collection
      const adminData = {
          id: newUser.uid,
          fullName: fullName,
          email: email,
      };
      const userDocRef = doc(firestore, 'admin_users', newUser.uid);
      await setDoc(userDocRef, adminData, { merge: true });

      toast({ title: "Account Created", description: "An administrator must grant you privileges. Redirecting..." });
      router.push('/admin/dashboard');
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen pt-20 pb-20 px-6 flex flex-col items-center justify-center">
      <div className="glass-panel p-10 max-w-md w-full border-primary/20 relative overflow-hidden bg-black/40">
        ...
        <Tabs defaultValue="login" className="w-full" onValueChange={() => { setShowPassword(false); setShowConfirmPassword(false); }}>
          <TabsList className="grid w-full grid-cols-2 bg-white/5 rounded-none p-1 border border-white/10">
            <TabsTrigger value="login" className="rounded-none font-headline text-[10px] tracking-widest py-3 uppercase">Login</TabsTrigger>
            <TabsTrigger value="signup" className="rounded-none font-headline text-[10px] tracking-widest py-3 uppercase">Sign Up</TabsTrigger>
          </TabsList>
            ...
            <form onSubmit={handleAdminSignUp} className="space-y-6 pt-6">
               <div className="space-y-2">
                  <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">Full Name</Label>
                   <div className="relative">
                    <Input name="fullName" type="text" required placeholder="Enter your full name" className="bg-white/5 border-white/10 rounded-none h-12 pl-4 pr-12 focus:border-primary transition-all" />
              ...
              <div className="space-y-2">
                <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">Confirm Security Key</Label>
                    <div className="relative">
                  <Input name="confirmPassword" type={showConfirmPassword ? "text" : "password"} required placeholder="••••••••" className="bg-white/5 border-white/10 rounded-none h-12 pl-4 pr-12 focus:border-primary transition-all" />
                  <Button
                    type="button"
                    variant="ghost"
                    size="icon"
              ...
              <Button type="submit" disabled={isSubmitting} className="w-full bg-accent text-background hover:bg-accent/80 py-6 font-headline tracking-[0.2em] rounded-none accent-glow">
